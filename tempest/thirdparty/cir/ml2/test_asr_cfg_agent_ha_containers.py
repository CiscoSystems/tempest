...
LOG.debug("TestASRCfgAgentHAContainers")


class TestASRCfgAgentHAContainers(test_network_multi_node.TestNetworkMultiNode):
    ...
    def setUp(self):
        ...
        self.log_inspectors = []
        self.log_inspectors.append(LogInspector())
        self.log_inspectors.append(LogInspector(log_dir='/opt/asr-cfg-agents/agent1/logs'))
        self.log_inspectors.append(LogInspector(log_dir='/opt/asr-cfg-agents/agent2/logs'))
        self.log_inspectors.append(LogInspector(log_dir='/opt/asr-cfg-agents/agent3/logs'))

        for log_inspector in self.log_inspectors:
            log_inspector.find_state_reports()
        #self.addCleanup(self.close_logs)
        self.addCleanup(self.check_log_errors)

        self.cfg_agent_host_names = [agent['host'] for agent in
                                     self.cisco_cfg_agents if agent['alive']]

        self.cfg_agent_ids = [agent['id'] for agent in
                              self.cisco_cfg_agents if agent['alive']]

        self.docker_cli = Client(base_url='unix://var/run/docker.sock')

        my_containers = self.docker_cli.containers()
        my_container_ids = [container['Id'] for container in my_containers]

        msg = "All active config-agents must be containers"
        for container_id in my_container_ids:
            cid = container_id[:12]
            self.assertIn(container_id[:12], self.cfg_agent_host_names, msg)

        msg = "All containers must be running at start of test"
        for cfg_agent_id in self.cfg_agent_host_names:
            cfg_agent_data = self.docker_cli.inspect_container(cfg_agent_id)
            self.assertEqual(cfg_agent_data['State']['Status'], 'running', msg)
    ...
    def check_log_errors(self):
        time.sleep(60)
        for log_inspector in self.log_inspectors:
            log_inspector.record_errors('test-end')
            log_inspector.compare_logs('baseline', 'test-end')
    ...
    def setup_multinode_network(self):
        ...
        self.security_group = self._create_security_group(tenant_id=
                                                          self.tenant_id)
    ...
    def delete_multinode_network(self, delete_aggregates=True):
    ...
    def failover_cfg_agent(self, cfg_agent, delay=90, verify_so=True):
        original_hosting_devices = \
            self.cfg_agent_client.list_hosting_device_handled_by_config_agent(
                cfg_agent_id=cfg_agent['id'])

        self.docker_cli.pause(cfg_agent['host'])
        time.sleep(delay)

        self.docker_cli.unpause(cfg_agent['host'])
        self.docker_cli.restart(cfg_agent['host'])
        time.sleep(delay)
        ...
        if verify_so:
            current_hosting_devices = \
                self.cfg_agent_client.list_hosting_device_handled_by_config_agent(
                    cfg_agent_id=cfg_agent['id'])
            self.assertNotEqual(original_hosting_devices,
                                current_hosting_devices)

    def setup_cfg_agent_ha(self, ha_type='MULTI_HA'):
        # Turn on all config agents
        for cfg_agent_controller in self.cfg_agent_host_names:
            agent_data = \
                self.docker_cli.inspect_container(cfg_agent_controller)

            if agent_data['State']['Status'] != 'running':
                self.docker_cli.unpause(cfg_agent_controller)
                self.docker_cli.restart(cfg_agent_controller)
                time.sleep(30)

        hosting_devices_req = self.cfg_agent_client.list_hosting_devices()
        self.assertEqual(hosting_devices_req.response['status'], '200',
                         hosting_devices_req.response)
        hosting_devices = hosting_devices_req['hosting_devices']
        self.assertEqual(len(hosting_devices), 2)

        cfg_agent1 = \
            self.cfg_agent_client.list_config_agents_handling_hosting_device(
                hosting_devices[0]['id'])
        self.assertEqual(cfg_agent1.response['status'], '200',
                         cfg_agent1.response)
        cfg_agent2 = \
            self.cfg_agent_client.list_config_agents_handling_hosting_device(
                hosting_devices[1]['id'])
        self.assertEqual(cfg_agent2.response['status'], '200',
                         cfg_agent2.response)

        if ha_type == 'MULTI_HA':
            if (cfg_agent1['agents'] and cfg_agent2['agents']) and \
                    cfg_agent1['agents'][0]['id'] == \
                            cfg_agent2['agents'][0]['id']:

                self.cfg_agent_client.config_agent_disassociate_hosting_device(
                    cfg_agent1['agents'][0]['id'], hosting_devices[0]['id'])
                self.docker_cli.restart(cfg_agent1['agents'][0]['host'])
            ...
            if (cfg_agent1['agents'] and cfg_agent2['agents']) and \
                ...
                self.cfg_agent_client.config_agent_disassociate_hosting_device(
                    cfg_agent1['agents'][0]['id'], hosting_devices[0]['id'])
                self.cfg_agent_client.config_agent_associate_hosting_device(
                    cfg_agent2['agents'][0]['id'], hosting_devices[0]['id'])
                self.docker_cli.restart(cfg_agent1['agents'][0]['host'])
        else:
            msg = "Invalid ha_type {0} specified".format(ha_type)
            raise asr_exceptions.ASRTestException(msg)

    def reset_all_cfg_agents(self):
        for cfg_agent_controller in self.cfg_agent_host_names:
            agent_data = \
                self.docker_cli.inspect_container(cfg_agent_controller)
    ...
    @test.attr(type='aaa-cfg-agent-ha-functional-containers')
    ...
    def test_standby_device_cfg_agent_failover(self):
        ...
        self.failover_cfg_agent(standby_cfg_agent[0])
    ...
    @test.attr(type='aaa-cfg-agent-ha-regress-containers')
    ...
    def test_net_create_during_active_failover(self):
        ...
        if not hasattr(self, 'max_instances_per_tenant'):
            self.max_instances_per_tenant = \
                CONF.scenario.max_instances_per_tenant
        self.max_instances_per_tenant = int(self.max_instances_per_tenant / 2)
        self.setup_multinode_network()
        self.create_floating_ips()
        time.sleep(60)
        self.verify_asrs_insync()
        ...
        self.failover_cfg_agent(active_cfg_agent[0], delay=5, verify_so=False)
        segmentation_ids = self.segmentation_ids
        self.setup_multinode_network()
        self.create_floating_ips()
        self.segmentation_ids = self.segmentation_ids + segmentation_ids
        self.verify_asrs_insync()
        self.verify_vm_connectivity()
        self.verify_vm_to_vm_connectivity()
    ...
    def test_net_delete_during_active_failover(self):
        ...
        active_cfg_agent = self.get_cfg_agent(active_ha_router)
        self.failover_cfg_agent(active_cfg_agent[0], delay=5, verify_so=False)
        self.delete_multinode_network(delete_aggregates=False)
    ...
    def test_net_create_during_standby_failure(self):
        ...
        standby_ha_router = self.get_ha_router(self.routers[0],
                                               ha_state='STANDBY')
        standby_cfg_agent = self.get_cfg_agent(standby_ha_router)
        self.failover_cfg_agent(standby_cfg_agent[0], delay=5, verify_so=False)
        self.setup_multinode_network()
        self.create_floating_ips()
        self.segmentation_ids = self.segmentation_ids + segmentation_ids
        self.verify_asrs_insync()
    ...
    def test_net_create_single_agent_failover(self):
        ...
        self.delete_multinode_network()
        time.sleep(30)
        self.failover_cfg_agent(active_cfg_agent[0])
        self.setup_multinode_network()
        self.create_floating_ips()
        time.sleep(90)
        self.verify_asrs_insync()
