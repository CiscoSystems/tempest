...
class TestNetworkMultiNode(manager.NetworkScenarioTest):
    ...
    * The VMs are then deleted, not as part of test cleanup but to allow the
        ...
        for hypervisor in self.hypervisors_list:
            if hypervisor['status'] == 'enabled':
                if hypervisor['state'] == 'up':
                    ...
                    i += 1
                    aggregate_list = self.aggregates_client.list_aggregates()['aggregates']
                    if len(aggregate_list) < i:
                        aggregate_resp = \
    ...
    def setUp(self):
        ...
        self.num_routers_per_tenant = CONF.scenario.num_routers_per_tenant
        ...
        LOG.debug("Max instances per project = {0}".
    ...
    def setup_networks(self):
        ...
        for i in range(0, self.num_networks):
            if len(self.routers) < int(self.num_routers_per_tenant):
    ...
    def setup_vms(self):
        ...
        for network in self.networks:
            for aggregate in self.aggregates:
                ...
                for i in range(0, 2):
                    try:
                        if CONF.scenario.use_host_aggregates is True:
                            server_dict = \
    ...
    @decorators.idempotent_id('094f246d-9800-4c79-b249-361dab5d5a0f')
